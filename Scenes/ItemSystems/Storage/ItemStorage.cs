...
public partial class ItemStorage : Node3D
{
    ...
    {
        if (_Quantity > 0) return;
        EmitSignal(nameof(OnStorageEmptied));
        _CurrentItem = "";
        _ItemSprite.Texture = null;
        _ItemLabel.Text = GetQuantityDisplay();
        _Collector.IDFilters = Array.Empty<string>(); // open filter to new items
    }
